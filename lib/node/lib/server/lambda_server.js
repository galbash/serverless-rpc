...
import { TLambdaServerTransport } from '../transport/lambda_transport';
 ...
 * @return {Function} The created Lambda server handler function
 */
export function createLambdaServer(processor, handler, options) {
  ...
  const server = new TLambdaServer(new RealProcessor(handler), options);
