...
describe('createLambdaConnection tests', () => {
  class LambdaStub {
    invoke = () => ({
      promise: self.invokeStub,
    })
  ...
  this.createLambdaConnection = createLambdaConnection;
  this.TLambdaConnection = TLambdaConnection;
  this.serverName = 'test-server-name';
  it('test sanity', () => {
    const conn = createLambdaConnection(this.serverName);
    expect(conn.serverName).to.equal(this.serverName);
  })
