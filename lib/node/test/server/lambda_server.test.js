...
import { TLambdaServer, createLambdaServer } from "../../lib";
...
describe('TLambdaServer tests', async () => {
  ...
  it('sanity', () => {
    let server = new TLambdaServer(this.processor);
    ...
    let args = this.processor.process.getCall(0).args;
...
describe('createLambdaServer tests', () => {
  ...
  it('use function', () => {
    ...
    let usedHandler = this.processor.getCall(0).args[0];
  ...
  it('use module', () => {
    ...
    expect(server).to.be.instanceOf(TLambdaServer);
    expect(this.processor.called).to.be.true;
    let [usedHandler] = this.processor.getCall(0).args;
    expect(usedHandler).to.equal(this.handler);
  });

  it('use options', () => {
    class StubProtocol {};
    let options = {protocol: StubProtocol};
